...
public class JsonIngestionStagingRepository : IIngestionStagingRepository
{
    ...
    public void SaveValidFindings(string jobId, List<FileFinding> validFindings)
    {
        ...
        _logger.LogInformation(
            "[STAGING_SAVE_START] JobId:{JobId}, Records:{Records}, Provider:Json",
            jobId,
            validFindings.Count);
        ...
        _logger.LogInformation(
            "[STAGING_SAVE_COMPLETE] JobId:{JobId}, Records:{Records}, Provider:Json",
            jobId,
            validFindings.Count);
    }
    ...
    public List<FileFinding> GetValidFindingsAfter(string jobId, int lastProcessedRecordCount)
    {
        ...
        _logger.LogInformation(
            "[STAGING_RESUME_READ] JobId:{JobId}, LastProcessedRecordCount:{LastProcessedRecordCount}, Provider:Json",
            jobId,
        ...
        lock (_lock)
        {
            ...
            _logger.LogInformation(
                "[STAGING_RESUME_READ_COMPLETE] JobId:{JobId}, LastProcessedRecordCount:{LastProcessedRecordCount}, Records:{Records}, Provider:Json",
                jobId,
                lastProcessedRecordCount,
                records.Count);
    ...
    private void WriteParquetWorkingFile(string jobId, List<FileFinding> validFindings)
    {
        ...
        _logger.LogInformation(
            "[PARQUET_STAGING_WRITE_START] JobId:{JobId}, InboundFileName:{InboundFileName}, Records:{Records}, Provider:Json",
            jobId,
            audit.InboundFileName,
            validFindings.Count);
        ...
        _logger.LogInformation(
            "[PARQUET_STAGING_WRITE_COMPLETE] JobId:{JobId}, Path:{Path}, Records:{Records}, Provider:Json",
            jobId,
            result.Path,
            result.RecordCount);
    }
    ...
    private List<FileFinding>? TryReadFromParquet(string jobId, int lastProcessedRecordCount)
    {
        ...
        var audit = _jobAuditRepository.GetByJobId(jobId);
        if (!IsParquetAvailable(audit))
            return null;

        try
        {
            _logger.LogInformation(
                "[PARQUET_RESUME_READ_ATTEMPT] JobId:{JobId}, Path:{Path}, LastProcessedRecordCount:{LastProcessedRecordCount}, Provider:Json",
                jobId,
                audit!.WorkingFilePath,
                lastProcessedRecordCount);
            ...
            {
                _logger.LogInformation("[PARQUET_RESUME_READ_SUCCESS] JobId:{JobId}, Records:{Records}, Provider:Json", jobId, records.Count);
                return records;
            }

            _logger.LogWarning("[PARQUET_RESUME_EMPTY_FALLBACK] JobId:{JobId}, Path:{Path}, Provider:Json", jobId, audit.WorkingFilePath);
            return null;
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "[PARQUET_RESUME_READ_FAILED] JobId:{JobId}. Falling back to JSON staging.", jobId);
            return null;
        }
    }
